...
import type { SurvivorConfig, SurvivorMatch } from "@prisma/client";

interface GameServerRoomSnapshot {
  matchId?: string;
  status?: string;
  startedAtMs?: number;
...
/**
 * After a Railway restart the Colyseus room forgets matchId while Postgres
 * still points at the open match. Re-bind the room before minting join tokens
 * so players aren't rejected with "Game server restarted".
 */
export async function ensureGameServerMatchBound(
  match: SurvivorMatch,
  ...
  let room: GameServerRoomSnapshot | null = null;
  try {
    ...
    if (!res.ok) return;
    const json = (await res.json()) as { room?: GameServerRoomSnapshot | null };
    room = json.room ?? null;
  } catch {
    return;
  }
  ...
  await fetch(`${baseUrl.replace(/\/$/, "")}/admin/rebind`, {
    method: "POST",
    headers: {
    ...
    console.error("[survivor] game server rebind failed", err);
  });
}
